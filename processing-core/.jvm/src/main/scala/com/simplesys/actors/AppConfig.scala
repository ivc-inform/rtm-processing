...
object AppConfig extends Config with Logging {
    ...
    val dsAdvisa = new OracleHikariDataSource(s"${config.getString("dbPool.defaultAdvisa")}.oraclcePoolDataSource")
    val dsLogger = new OracleHikariDataSource(s"${config.getString("dbPool.defaultLogging")}.oraclcePoolDataSource")
    val dsScenario = new OracleHikariDataSource(s"${config.getString("dbPool.defaultScenario")}.oraclcePoolDataSource")
