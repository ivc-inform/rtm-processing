...
import ru.mfms.mfmd.integration.classifiertypes.TextType
import ru.mfms.mfmd.integration.templates.ReasonToStage
import ru.mfms.mfmd.integration.templates.ReasonToStage.{NoDataExtracted, NoTemplatesFound, TokenizerError, TooManyTemplates}
import ru.simplesys.dmprocess.classifier.ClassifierModel
...
class PhoneHolderActor(
    ...
    def getData(msg: MessageMFMD): Either[ReasonToStage, (FixedTemplate, List[CategoryParamRefWValue])] = {
        ...
        preprocessedTokensValidation match {
            case Right(preparedTokens) =>
                val (statOrigin, tryFixed: List[FixedTemplate]) = templateHolder.lookupTemplates(preparedTokens)
                val datas = TemplateUtils.getDatas(tryFixed, preparedTokens)
                datas match {
                    case (x@(t, vals)) :: Nil =>
                        Right(x)
            ...
            case Left(_) => Left(TokenizerError)
        }
    }
