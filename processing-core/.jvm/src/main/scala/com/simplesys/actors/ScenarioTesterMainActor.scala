                ...
                idGroupTest ⇒
                    updateStatus(StatusScenario.stoped.value, idGroupTest).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                        case -\/(e) =>
                            throw e
                        case \/-(a) =>
                            SendMessage(Message(data = obj("idGroupTest" → fromLong(idGroupTest), "value" → fromInt(StatusScenario.stoped.value)), channels = refteshTestGroupGridMessage))
                    }
            }
    }
}
...
class ScenarioTesterActor(val service: ComfmdiConnectorMfmdOutMessageServiceImpl) extends Actor with Logging {
    ...
    override def receive: Receive = {
                    ...
                    try {
                        ...
                        queue.zipWithIndex.foreach {
                            case (message, index) =>
                                ...
                                groups.find(_.idGroupTest == message.idGroupTest) match {
                                    case None ⇒
                                        ...
                                        updateStatus(StatusScenario.play.value, message.idGroupTest).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                            ...
                                            case \/-(a) =>
                                                SendMessage(Message(data = obj("idGroupTest" → fromLong(message.idGroupTest), "value" → fromInt(StatusScenario.play.value)), channels = refteshTestGroupGridMessage))
                                                ...
                                                selectTestGroup(message.idGroupTest).unique.transact(AppConfig.dsScenarioTransactor).runAsync {
                                                    ...
                                                    case \/-(groupTestRecord) =>
                                                        SendMessage(Message(data = obj(
                                                            "idGroupTest" → fromLong(message.idGroupTest),
                                                            "value" → fromInt(StatusScenario.play.value),
                                                            "codeGroup" → fromString(groupTestRecord.codeGroup),
                                                            "captionGroup" → (if (groupTestRecord.captionGroup.isEmpty) Null else fromString(groupTestRecord.captionGroup.get))
                                                        ), channels = openLogWindowMessage))
                                ...
                                def makeStep(index: Int, groupIndex: Int): Unit = {
                                    logger info s"!!!!! GroupIndex: $groupIndex !!!!!!!!".append2Channel(updateContentLogWindowMessage(message.idGroupTest))
                                    if (index != 0) {
                                        val t0: Long = queue(index).timestamp.getMillis - queue(index - 1).timestamp.getMillis
                                        //val t = Duration(t0, TimeUnit.MILLISECONDS)
                                        logger warning s"Time 0 : ${DT(t0)}".append2Channel(updateContentLogWindowMessage(message.idGroupTest))
                                        Thread sleep t0
                                    }
                                    logger debug s"${newLine.newLine} ////////////////////////////////////////////// $newLine Sended message: ${message.toString.newLine}".append2Channel(updateContentLogWindowMessage(message.idGroupTest))

                                    service processMfmdOutMessage message
                                    logger info s"!!!!! End GroupIndex: $groupIndex !!!!!!!!".newLine.append2Channel(updateContentLogWindowMessage(message.idGroupTest))
                                }
                        }

                        groups.foreach {
                            group ⇒
                                logger info s"(${group.qty}) Messages sent !!!!!!!!".append2Channel(updateContentLogWindowMessage(group.idGroupTest))
                                updateStatus(StatusScenario.stoped.value, group.idGroupTest).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                    case -\/(e) =>
                                        throw e

                                    case \/-(a) =>
                                        SendMessage(Message(data = obj("idGroupTest" → fromLong(group.idGroupTest), "value" → fromLong(StatusScenario.stoped.value)), channels = refteshTestGroupGridMessage))
                        ...
                        case e: Throwable ⇒
                            SendMessage(Message(data = obj(
                                ...
                                "stackTrace" → fromString(e.getStackTrace().mkString("", newLine, newLine))
        ...
        case TestStop(idsGroupTest) ⇒
            idsGroupTest.foreach(idsGroup ⇒ groups.map(_.idGroupTest).toSeq.intersect(Seq(idsGroupTest)).foreach (idGroupTest ⇒ groups.find(_.idGroupTest == idsGroup).foreach(_.break = true)))
    }
}
