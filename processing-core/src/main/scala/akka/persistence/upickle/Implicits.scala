...
trait Implicits extends Logging {
    ...
    implicit val dateTime2Writer: Writer[DateTime] = Writer[DateTime](dateTime => Js.Str(dateTime.toString))
    implicit val dateTime2Reader: Reader[DateTime] = Reader[DateTime] { case Js.Str(date) => DateTime.parse(date) }
