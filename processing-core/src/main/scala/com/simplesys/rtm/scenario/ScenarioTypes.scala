...
abstract trait AbstractPersistedTimer extends AbstractEvent with Logging {
    ...
    def alarmAt: DateTime = new DateTime(startedAt).plus(duration.toMillis)

    def isAlarmInFuture = alarmAt.isAfterNow

    def timeout: FiniteDuration = FiniteDuration(alarmAt.getMillis - DateTime.now().getMillis, TimeUnit.MILLISECONDS)
...
case class TimerUnified(name: String, cancelIt: Boolean, startedAt: DateTime, startOverdueTimer: Boolean, duration: FiniteDuration, scenario: ScenarioData) extends AbstractPersistedTimer
