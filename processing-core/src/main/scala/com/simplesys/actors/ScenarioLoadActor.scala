...
trait Loader extends ScenarioRegistry with Config {
    ...
    def loadDebugScenarios(): Unit = {
        ...
        def select: Query0[ScenarioTableRecordEx] = {
            val q = fr"""SELECT  RTM_SCR_CMPGN.SCODE_CMPGN,
                            ...
                            case None ⇒
                                ...
                                loadScenarioCompanion(scenarioCompanionName, code) match {
                                    case Success(scenarioCompanion) ⇒
                                        ...
                                        updateStatus(new com.simplesys.js.components.Play(StatusScenario.getStatus(record.status).test, StatusScenario.getStatus(record.status).prod).value, record.idScenario).run.transact(AppConfig.dsScenarioTransactor).run
...
abstract class ScenarioLoadActor extends Actor with Config with Logging {
    ...
    def prepare(idsScenario: NonEmptyList[Long], testModeGenerate: Boolean = false, prodTestModeGenerate: Boolean = false)(implicit eval: EvalExt): Unit = {
                        ...
                        case Failure(e: GenerateException) ⇒
                            updateStatus(StatusScenario.notReady.value, scenarioTableRecord.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                case -\/(e) =>
                                    throw e
                                case \/-(a) =>
                                    SendMessage(Message(data = JsonObject.apply1("error" → e.message), channels = appCommon.refteshScenarioGridMessage))
                            }
                            None
                        ...
                        case Failure(e) ⇒
                            updateStatus(StatusScenario.notReady.value, scenarioTableRecord.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                case -\/(e) =>
                                    throw e
                                case \/-(a) =>
                                    SendMessage(Message(data = JsonObject.apply1("error" → e.getMessage), channels = appCommon.refteshScenarioGridMessage))
                            }
                            None
                                ...
                                case Success(scenarioCompanion) ⇒
                                    ...
                                    updateClobExe(Some(code), generatedCodeItem.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                        ...
                                        case \/-(a) =>
                                            ...
                                            SendMessage(Message(appCommon.refteshScenarioGridMessage))
                                    }
                                ...
                                case Failure(e: Throwable) ⇒
                                    ...
                                    updateStatus(StatusScenario.notReady.value, generatedCodeItem.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                        case -\/(e) =>
                                            ...
                                            throw e
                                        case \/-(a) =>
                                            SendMessage(Message(data = JsonObject.apply1("error" → eval.getErrorsString), channels = appCommon.refteshScenarioGridMessage))
                                    }
                            }
                        case Failure(e: GenerateException) ⇒
                            updateStatus(StatusScenario.notReady.value, scenarioTableRecord.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
        ...
        SendMessage(Message(appCommon.makeOperationMessage))
    }
                    ...
                    json ⇒
                        ...
                        loadScenarioCompanion(scenarioCompanionName, code) match {
                            case Success(scenarioCompanion) ⇒
                                ...
                                updateStatus(new com.simplesys.js.components.Play(StatusScenario.getStatus(record.status).test, StatusScenario.getStatus(record.status).prod).value, record.idScenario).run.transact(AppConfig.dsScenarioTransactor).runAsync {
                                    case -\/(e) =>
                                        throw e
                                    case \/-(a) =>
                                        SendMessage(Message(channels = appCommon.refteshScenarioGridMessage))
                                }
                            case Failure(e) ⇒
