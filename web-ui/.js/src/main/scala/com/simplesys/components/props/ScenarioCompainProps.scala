...
import com.simplesys.app.App.{Compain, ScenarioStatistics, TabRTM}
...
class ScenarioCompainProps extends CommonTreeListGridEditorComponentProps {
    ...
    def makeOperation(idScenario: JSUndefined[Double], _mode: StatusScenario, testMode: JSUndefined[Boolean] = false, prodTestMode: JSUndefined[Boolean] = false): Unit = {
        ...
        isc.MessagingSS.subscribe(makeOperationMessage, {
            (messageJS: MessageJS) ⇒
                info.foreach(_.markForDestroy())
            //isc.MessagingSS.unsubscribe(makeOperationMessage)
        })
        ...
        new MenuSSItemProps {
            ...
            click = {
                (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
                    val editor = item.owner.asInstanceOf[GridEditor[ListGridField, ScenariosScr_ScenarioDataRecord, ListGridSelectedState]]
            ...
            enableIf = {
                (target: Canvas, menu: MenuSS, item: MenuSSItem) =>
                    val owner = item.owner.asInstanceOf[GridEditor[ListGridField, ScenariosScr_ScenarioDataRecord, ListGridSelectedState]]
    ...
    def getViewWindow(_title: String, selectedRecord: ScenariosScr_ScenarioDataRecord, icon: SCImgURL, code: String, error: String): Unit = {
    ...
    initWidget = {
        (thisTop: classHandler, arguments: IscArray[JSAny]) =>
                    ...
                    MenuSSItem(new MenuSSItemProps {
                        ...
                        click = {
                            (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
                                val editor = item.owner.asInstanceOf[GridEditor[ListGridField, ScenariosScr_ScenarioDataRecord, ListGridSelectedState]]
                                simpleSyS checkOwner editor
                                val selectedRecord: ScenariosScr_ScenarioDataRecord = editor.getSelectedRecord()
            ...
            isc.MessagingSS.subscribe(refteshScenarioGridMessage, {
                (messageJS: MessageJS) ⇒
            ...
            thisTop.observe(thisTop, "destroy", {
                () ⇒
                    ...
                    isc.MessagingSS unsubscribe refteshScenarioGridMessage
    ...
    def getButtonProps(record: ScenariosScr_ScenarioDataRecord, listGridEditor: ListGridEditor) = new IButtonSSProps {
        height = 20
        width = 130
        ...
        (thiz: ListGridEditor, record: ListGridRecord, colNum: Int) =>
            ...
            fieldName match {
                case x if x == scenarios_Scr_Scenario_status_NameStrong.name ⇒
                    val _record = record.asInstanceOf[ScenariosScr_ScenarioDataRecord]
