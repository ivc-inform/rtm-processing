...
import com.simplesys.app.ScenarioTestsGroups
import com.simplesys.appCommon
import com.simplesys.container.FullHandContainer.ScenarioTesterContainer
import com.simplesys.function._
import com.simplesys.js.components.{ScenarioTests, StatusScenario, StatusTestScenario}
import com.simplesys.option.DoubleType._
import com.simplesys.option.ScOption
import com.simplesys.option.ScOption._
import ru.simplesys.defs.app.gen.scala.ScalaJSGen._
import ru.simplesys.defs.app.scala.container.TestGroupDataRecord
...
class LoggingViewer extends JSObject {
    ...
    def getWindowIdentifier(idGroupTest: Double) = s"${appCommon.loggingWindowGroupID}_$idGroupTest"
    ...
    def subscribe4Update(idGroupTest: Double): Unit = {
        ...
        isc.MessagingSS.subscribe(appCommon.updateContentLogWindowMessage(idGroupTest), {
            (messageJS: MessageJS) ⇒
    ...
    def unSubscribe4Update(idGroupTest: Double): Unit = {
        logs.find { case (key, _) ⇒ key == idGroupTest.toLong.toString }.foreach {
            case (key, _) ⇒
                isc.MessagingSS.unsubscribe(appCommon.updateContentLogWindowMessage(idGroupTest))
        }
    }
    ...
    def getViewWindows(selectedRecords: TestGroupDataRecord*): Unit = selectedRecords.foreach {
        selectedRecord ⇒
...
class ScenarioTestsProps extends CommonTreeListGridEditorComponentProps {
    ...
    def seqMenuControlsItemProps(owner: TreeGridEditor) = Seq(
        new MenuSSItemProps {
            ...
            click = {
                (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
                    val editor = item.owner.asInstanceOf[GridEditor[ListGridField, TestGroupDataRecord, ListGridSelectedState]]
                    simpleSyS checkOwner editor
                    makeTest(editor, StatusTestScenario.play)
            }.toFunc.opt
            enableIf = {
                (target: Canvas, menu: MenuSS, item: MenuSSItem) =>
                    val owner = item.owner.asInstanceOf[GridEditor[ListGridField, TestGroupDataRecord, ListGridSelectedState]]
        ...
        new MenuSSItemProps {
            ...
            click = {
                (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
                    val editor: GridEditor[ListGridField, TestGroupDataRecord, ListGridSelectedState] = item.owner.asInstanceOf[GridEditor[ListGridField, TestGroupDataRecord, ListGridSelectedState]]
                    simpleSyS checkOwner editor
                  ...
                  IscArray(
                      ...
                      MenuSSItem(
                          new MenuSSItemProps {
                              ...
                              identifier = "logView".opt
                              icon = appCommon.iconDiscard.opt
                              click = {
                                  (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
    ...
    def getButtonProps(record: TestGroupDataRecord, listGridEditor: TreeGridEditor) = {
    ...
    createTreeRecordComponent = {
        (thiz: TreeGridEditor, record: TestGroupDataRecord, colNum: Int) =>
    ...
    updateTreeRecordComponent = {
        (thiz: TreeGridEditor, record: TestGroupDataRecord, colNum: Int, component: Canvas, recordChanged: Boolean) ⇒
    ...
    def makeTest(editor: GridEditor[ListGridField, TestGroupDataRecord, ListGridSelectedState], _mode: StatusScenario): Unit = {
        //isc debugTrap editor
        val _idsGroupTest: JSArray[Double] = editor.getSelectedRecords().map(_.id.get)

        isc.MessagingSS.subscribe(appCommon.errorMessage, {
            (messageJS: MessageJS) ⇒
                messageJS.data.foreach {
                    item ⇒
        ...
        isc.MessagingSS.subscribe(appCommon.openLogWindowMessage, {
            (messageJS: MessageJS) ⇒
                messageJS.data.foreach {
                    item ⇒
        ...
        isc.MessagingSS.subscribe(appCommon.refteshTestGroupGridMessage,
            (messageJS: MessageJS) ⇒
                messageJS.data.foreach {
                    item ⇒
                        val data = item.asInstanceOf[RefreshTestGroupGridResult]
                            ...
                            case (idGroupTest, value) ⇒
                                value match {
                                    case value if value == StatusTestScenario.stoped.value ⇒
                                        isc.MessagingSS unsubscribe (IscArray(appCommon.openLogWindowMessage, appCommon.errorMessage, appCommon.refteshTestGroupGridMessage))
                        ...
                        if (isc.isA.TreeGridEditor(editor)) {
                            ...
                            selectedRec.asInstanceOf[JSDynamic].updateDynamic("status")(data.value)
                            editor.applyRecordData(IscArray(selectedRec))
        ...
        RPCManagerSS.sendRequest(
            RPCRequest(
                new RPCRequestProps {
                    ...
                    actionURL = (simpleSyS.simpleSysContextPath + ScenarioTesterContainer.scenarios_Scr_Scenario_ScenarioTest).opt
                    callback = {
