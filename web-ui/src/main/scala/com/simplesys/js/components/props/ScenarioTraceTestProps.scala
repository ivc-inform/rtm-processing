...
import com.simplesys.js.components.{ScenarioTrace, ScenarioTraceTest}
import com.simplesys.option.ScOption._
import ru.simplesys.defs.app.gen.scala.ScalaJSGen.{DataSourcesJS, FormItemsJS, ListGridFiledsJS}

class ScenarioTraceTestProps extends CommonListGridEditorComponentProps {
    type classHandler <: ScenarioTraceTest
    ...
    identifier = "C4C555D1-7823-F191-BD54-11A362238DCC".opt
    dataSource = DataSourcesJS.scenarios_ScenarioTrace_Debug_DS.opt
    fields = ListGridFiledsJS.scenarios_ScenarioTrace_Debug_FLDS.opt
    editingFields = FormItemsJS.scenarios_ScenarioTrace_Debug_FRMITM.opt
    customMenu = Seq(
                            ...
                            new MenuSSItemProps {
                                ...
                                click = {
                                    (target: Canvas, item: MenuSSItem, menu: MenuSS, colNum: JSUndefined[Int]) =>
                                        item.owner.foreach {
                                            owner ⇒
                                                val _owner = owner.asInstanceOf[ScenarioTraceTest]
                                                _owner.exportData()
                                        }
