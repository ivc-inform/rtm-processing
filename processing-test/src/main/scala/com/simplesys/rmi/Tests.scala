...
object Tests extends App with Config with Logging {
    ...
    def start(config: ProcessingSbtTestConfig) {
        try {
                    ...
                    case com.simplesys.scopt.runTestQueue =>
                        ...
                        s"""select * from
                          ...
                          |from
                            ...
                            | and c.cnr_id = cnr.id
                            | and c.our_id=ort.id
                            | and c.TIMESTAMP between to_date('${dateToStr(config.startDate)}','dd.mm.yyyy hh24:mi:ss') and to_date('${dateToStr(config.endDate)}','dd.mm.yyyy hh24:mi:ss')
                            |
            ...
            val proc = HC.process[MessageMFMD](sql, HPS.setFetchSize(dsLogger.Config.FetchSize), dsLogger.Config.FetchSize)
            ...
            queue.zipWithIndex.foreach {
                case (message, index) =>
                    if (index != 0) {
                        val t0 = queue(index).timestamp.toDateTime.getMillis - queue(index - 1).timestamp.toDateTime.getMillis
